...
//    int offx, offy;
//    double distance;
//    for(r=1;r<radius*4;r++) {
//      rint=(int)r/4;
//      power = 1;
//      x = 0;
//      y = rint;
//      distance = 5.0 / 4.0 - r/4.0;
//      //    lightArray[radius + radius * diameter] = color;
//      //    for (int i = 0; i < lightArray.length; i++)
//      //      lightArray[i] = 0xffffff00;
//      while (y >= x) {
//        if (distance < 0)
//          distance += 2.0 * x + 3.0;
//        else {
...
//        lightArray[x + offx + rint - 1 + (rint + y + offy - 1) * diameter] = color; // done
//        lightArray[(-x + offx + rint) + (rint + y + offy - 1) * diameter] = color; // done
//        lightArray[(x + offx +rint - 1) + (rint+ -y + offy) * diameter] = color; // done
//        lightArray[(-x + offx + rint) + (rint + -y + offy) * diameter] = color; // done
//        lightArray[y + offx + rint - 1 + (rint + x - 1 + offy) * diameter] = color; // done
//        lightArray[(y + offx + rint - 1) + (rint + -x + offy) * diameter] = color; // done
//        lightArray[(-y + offx + rint) + (rint + x + offy) * diameter] = color; // done
//        lightArray[((-y + offx + rint) + (rint + -x + 1 + offy) * diameter)] = color; // done
//
//      }
//    }
// start
    ...
    for (int filly = 1; filly < diameter - 1; filly++ )
    {
      for (int fillx = 1; fillx < diameter; fillx++)
      {
        ...
        double power = 0.78 - dxdy / (diameter2); // is nu tussen 0 en 1.
...
//        System.out.println(1 - distance / radius);
//        if (distance < radius)
